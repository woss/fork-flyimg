...
SVG;
    ...
    public function __construct(
        ...
        $this->logger = $logger;
        $this->fileLinkFormat = $fileLinkFormat instanceof FileLinkFormatter
            ? $fileLinkFormat
            : new FileLinkFormatter($fileLinkFormat);
    }
    ...
    private function renderException(FlattenException $exception): string
    {
        ...
        return $this->include(
            ...
            'statusText' => $statusText,
            'statusCode' => $statusCode,
            'logger' => null !== $this->logger && class_exists(DebugLoggerConfigurator::class)
                ? DebugLoggerConfigurator::getDebugLogger($this->logger)
                : null,
            'currentContent' => \is_string($this->outputBuffer)
                ? $this->outputBuffer
                : ($this->outputBuffer)(),
            ]
        );
    ...
    private function include(string $name, array $context = []): string
    {
        ...
        include is_file(\dirname(__DIR__) . '/Views/' . $name) ? \dirname(__DIR__) . '/Views/' . $name : $name;
    ...
    private function formatFileFromText(string $text): string
    {
        return preg_replace_callback(
            '/in ("|&quot;)?(.+?)\1(?: +(?:on|at))? +line (\d+)/s',
            fn ($match) => 'in ' . $this->formatFile($match[2], $match[3]),
            $text
        ) ?? $text;
    }
    ...
    private function formatFile(string $file, int $line, ?string $text = null): string
    {
        ...
        return sprintf(
            '<a href="%s" title="Click to open this file" class="file_link">%s</a>',
            $this->escape($link),
            $text
        );
    }
    ...
    private function fileExcerpt(string $file, int $line, int $srcContext = 3): string
    {
        if (is_file($file) && is_readable($file)) {
            ...
            if (\PHP_VERSION_ID >= 80300) {
                ...
                $code = preg_replace_callback(
                    '#<span ([^>]++)>((?:[^<]*+<br \/>)++[^<]*+)</span>#',
                    fn ($m) => "<span $m[1]>" . str_replace('<br />', "</span><br /><span $m[1]>", $m[2]) . '</span>',
                    $code
                );
            ...
            for (
                $i = max($line - $srcContext, 1), $max = min($line + $srcContext, \count($content));
                $i <= $max; ++$i
            ) {
                $lines[] = '<li' . ($i == $line ? ' class="selected"' : '') . '><code>'
                    . $this->fixCodeMarkup($content[$i - 1]) . '</code></li>';
            }
