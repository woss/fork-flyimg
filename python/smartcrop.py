...
class SmartCrop(object):
    ...
    def detect_saturation(self, cie_array, source_image):
        ...
        saturation_data[mask] = (
            saturation_data[mask] - threshold) * (255 / (1 - threshold))
...
def parse_argument():
    ...
    parser.add_argument('--width', dest='width', type=int,
                        default=100, help='Crop width')
    parser.add_argument('--height', dest='height', type=int,
                        default=100, help='Crop height')
...
def main():
    ...
    result = cropper.crop(image, width=100, height=int(
        options.height / options.width * 100))
